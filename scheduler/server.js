...
function parseManagedCronLine(line) {
  ...
  const isPaused =
    trimmed.startsWith(PAUSED_PREFIX) ||
    (trimmed.startsWith('#') && trimmed.includes(SCRIPT_PATH))
...
app.post('/api/schedules', requireAuth, (req, res) => {
  ...
  const existing = schedules.find(
    (s) => s.draftFile === draftFile && s.status === 'pending'
  )
  if (existing) {
    return res.status(409).json({
      error: 'A pending schedule already exists for this draft',
      existing,
    })
  }
...
app.listen(PORT, '127.0.0.1', () => {
  console.log('[aivoicereview-scheduler] API running on port ' + PORT)
  console.log('[aivoicereview-scheduler] Data file: ' + DATA_FILE)
})
